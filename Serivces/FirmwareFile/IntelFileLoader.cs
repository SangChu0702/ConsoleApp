...
namespace Core.FirmwareFile
{
    ...
    public static class IntelFileLoader
    {
        ...
        private class Record
        {
            ...
            public Record(RecordType type, uint address, byte[] data)
            {
                Type = type;
                Address = address;
                Data = data;
        ...
        private static uint GetExtendedLinearAddress(Record record)
        {
            ...
            return ((uint)record.Data[0] << 24) + ((uint)record.Data[1] << 16);
        }
