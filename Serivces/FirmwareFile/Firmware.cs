...
namespace Core.FirmwareFile
{
    ...
    public class Firmware
    {
         ...
         */
        public void SetData(uint startAddress, byte[] data)
        {
            if (data.Length == 0)
            {
                return;
         ...
         */
        public void EraseData(uint startAddress, uint size)
        {
            if (size == 0)
            {
                return;
         ...
         */
        public byte[]? GetData(uint startAddress, uint size)
        {
            uint endAddress = startAddress + size;
        ...
        private void RemoveOverwrittenBlocks(uint startAddress, uint endAddress)
        {
            foreach (var block in m_blocks.ToArray())
            {
